...
function tap_http_rq.packet()
	method = tostring(method_f())
	if  string.upper(method) == "GET" then C_GET = C_GET + 1
	elseif  string.upper(method) == "POST" then C_POST = C_POST + 1
	elseif  string.upper(method) == "HEAD" then C_POST = C_POST + 1
...
function tap_http_rp.packet()
  ...
  if (code >= 100 and code <= 199) then R_100 = R_100 + 1
  elseif (code >= 200 and code <= 299) then R_200 = R_200 + 1
