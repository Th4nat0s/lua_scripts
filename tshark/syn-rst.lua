-- Find syn requests answered by RST 
...
-- debug ( 'Client ' .. tostring(client_s()) .. ' Syn'.. tostring(Syn() ) .. ' Ack'.. tostring(Ack()) .. " " ..  tostring(Port_s()) )
  if ((tostring(Ack()) == "0") and (tostring(client_s()) == MYCLIENT)) then
     Port = MYCLIENT ..':'.. tostring(Port_s()) .. '>' .. tostring(client_d()) .. ':' .. tostring(Port_d())
    if IP[Port] == nil then
      IP[Port] = 1
    else
      IP[Port] = IP[Port] + 1
 ...
 if ((tostring(Ack()) == "1") and (tostring(client_d()) == MYCLIENT)) then
--  # debug (Port .. "+++" ..tostring(Syn()) .. "++++" .. tostring(Ack()))
  Port =  MYCLIENT ..':'.. tostring(Port_d()) .. '>' .. tostring(client_s()) .. ':' .. tostring(Port_s())
if IP[Port] == nil then
      IP[Port] = -1
    else
       IP[Port] = IP[Port] - 1
